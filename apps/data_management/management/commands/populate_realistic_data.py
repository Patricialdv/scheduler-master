...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Crear horarios base (placeholder) para períodos activos
        self._create_schedule_placeholders()
    ...
    def _create_period(self, career: str, number: int) -> Period:
        ...
        # Lógica Par/Impar: períodos pares (2,4,6,8,10) activos, impares inactivos
        is_even_period = (number % 2 == 0)
        ...
        period, created = Period.objects.get_or_create(
            ...
            defaults={
                "weeks_count": 16,
                "is_active": is_even_period,  # PARES activos, IMPARES inactivos
                "start_date": start_date,
            }
        )
        # Actualizar si ya existía para asegurar consistencia
        if not created and period.is_active != is_even_period:
            period.is_active = is_even_period
    ...
    def _create_schedule_placeholders(self):
        """Crea horarios base (placeholder) para períodos activos sin Schedule."""
        from apps.data_management.models import Schedule, Group
        
        self.stdout.write(self.style.WARNING("\n[7/7] Creando horarios base..."))
        
        active_periods = Period.objects.filter(is_active=True).prefetch_related('groups')
        placeholder_count = 0
        
        for period in active_periods:
            for group in period.groups.all():
                # Solo crear si no existe ya un Schedule
                if not Schedule.objects.filter(period=period, group=group, is_base=True).exists():
                    Schedule.objects.create(
                        period=period,
                        group=group,
                        is_base=True,
                        score=0,  # Score 0 indica que necesita generación
                    )
                    placeholder_count += 1
        
        if placeholder_count > 0:
            self.stdout.write(self.style.SUCCESS(
                f"  ✓ {placeholder_count} horarios base creados (pendientes de generación)"
            ))
        else:
            self.stdout.write(self.style.WARNING(
                "  ⚠ No se crearon horarios - ya existen o no hay períodos activos"
            ))